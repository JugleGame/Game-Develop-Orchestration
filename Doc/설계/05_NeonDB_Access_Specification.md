...
| 후보 | 필요해지는 조건 | 권고 |
|---|---|---|
| StrategicMcpServer | 자매 레포의 리서치 카드 미러(`cards` + `card_sections` 테이블)를 장르/요소 레퍼런스로 **직접 조회**하기로 결정할 때 | 별도 **읽기 전용 롤**로 분리 접속. Job 테이블 접근 금지. 회수 단위는 카드가 아니라 **절**(`card_sections`, pgvector 1024차원 / `BAAI/bge-m3`)이다 — 스키마 v2, 2026-08-12 |
